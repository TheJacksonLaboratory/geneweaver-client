...
def to_csv(batch_file: Path,
           output_directory: Optional[Path] = None,
           prefix: Optional[str] = None,
           geneset_ids: Optional[List[str]] = None,
           read_file: bool = True) -> Optional[List[Path]]:
    ...
    csv_files = [write_geneset_to_csv(geneset,
                                      None,
                                      output_directory,
                                      prefix,
                                      gs_id)
...
def to_csv_indexed(batch_file: Path,
                   index_file: Path,
                   output_directory: Optional[Path] = None):
    ...
    for geneset in genesets:
        try:
            ...
            filename = write_geneset_to_csv(geneset,
                                            uberon_id,
                                            output_directory,
                                            disease,
                                            gs_id)
...
def write_geneset_to_csv(geneset: BatchUploadGeneset,
    ...
    header = [(f'{header_prefix}{key}', value) for key, value in geneset.dict(exclude={"values"}).items()]
